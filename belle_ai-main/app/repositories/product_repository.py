...
def get_product_by_sku(db: Session, sku: str) -> Optional[Product]:
    ...
    if product:
        logger.info(
            f"[REPOSITORY] ✓ Product found: id={product.id}, name={product.name}, price={product.price}, tags={product.tags}"
        )
    else:
...
def get_product_by_brand_and_sku(
    ...
    if product:
        ...
        logger.warning(
            f"[REPOSITORY] ✗ Product not found: brand_code={brand_code}, sku={sku}"
        )
    return product
...
def upsert_product_by_brand_and_sku(
        ...
        if field in ("tags", "attributes") and value is not None:
            if isinstance(value, (dict, list)):
                ...
                try:
                    ...
                    logger.warning(
                        f"[REPOSITORY] Invalid JSON string for {field}: {value}"
                    )
                    values[field] = None
            else:
                values[field] = value
    ...
    if not product:
        raise RuntimeError(
            f"Failed to upsert product: brand_code={brand_code}, sku={sku}"
        )
...
def get_candidate_products_by_brand(
    ...
    if check_on_sale:
        try:
            ...
            logger.debug(
                f"[REPOSITORY] on_sale column check failed: {e}, skipping filter"
            )
    ...
    if category:
        ...
        for product in products:
            ...
            if hasattr(product, "category") and product.category:
                ...
                product_category = product.attributes.get(
                    "category"
                ) or product.attributes.get("类目")
        ...
        logger.debug(
            f"[REPOSITORY] Filtered by category in Python: {len(products)} products"
        )
